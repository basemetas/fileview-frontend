  ...
  function Element(str) {
    ...
    if (attrList != null) {
      for (var key in attrList) {
        ...
        _this.attributeList[attrKey] = attrValue.substr(
          1,
          attrValue.length - 2
        );
      }
...
var LuckySheetCelldata = /** @class */ (function (_super) {
  ...
  LuckySheetCelldata.prototype.generateValue = function (s, t) {
    ...
    if (s != null) {
      ...
      if (xfId != null && cellStyleXfs[parseInt(xfId)]) {
  ...
  LuckySheetCelldata.prototype.getBackgroundByFill = function (
    fill,
    clrScheme
  ) {
...
var LuckySheet = /** @class */ (function (_super) {
    ...
    if (drawingFile != null && drawingRelsFile != null) {
      ...
      if (twoCellAnchors != null && twoCellAnchors.length > 0) {
        for (var i = 0; i < twoCellAnchors.length; i++) {
          ...
          // TODO: 此处只处理了图表
          var chartData = twoCellAnchor.getInnerElements('c:chart');
          if (
            xdrFroms != null &&
            xdrFroms.length > 0 &&
            chartData != null &&
            ...
            var rembed = getXmlAttibute(
              chartDataItem.attributeList,
              'r:id',
              null
            );
            // console.log('chartDataItem ===> ', chartDataItem);
            // console.log('rembed ===> ', rembed);
            // console.log('drawingRelsFile ==> ', drawingRelsFile);
            var chartObject = _this.getChartByRid(rembed, drawingRelsFile);
            ...
            if (chartObject != null) {
              ...
              chartObject.fromCol = _this.getXdrValue(
                xdrFrom.getInnerElements('xdr:col')
              );
              chartObject.fromRow = _this.getXdrValue(
                xdrFrom.getInnerElements('xdr:row')
              );
              chartObject.toCol = _this.getXdrValue(
                xdrTo.getInnerElements('xdr:col')
              );
              chartObject.toRow = _this.getXdrValue(
              ...
              _this.chart.push(chartObject);
              
              console.log('✅ 图表已添加到 sheet.chart:', chartObject);
            }
          }
  ...
  LuckySheet.prototype.getChartByRid = function (rid, drawingRelsFile) {
    var Relationships = this.readXml.getElementsByTagName(
      'Relationships/Relationship',
      drawingRelsFile
    );
    if (Relationships != null && Relationships.length > 0) {
      for (var i = 0; i < Relationships.length; i++) {
        ...
        if (Id == rid) {
          ...
          // 读取图表 XML 对象
          var chartXml = this.readXml.getElementsByTagName('c:chartSpace', src);
          if (chartXml != null && chartXml.length > 0) {
            var chartSpace = chartXml[0];
            
            // 解析图表类型
            var chartType = null;
            var plotArea = this.readXml.getElementsByTagName('c:chartSpace/c:chart/c:plotArea', src);
            if (plotArea != null && plotArea.length > 0) {
              var plotAreaElement = plotArea[0];

              console.log('plotAreaElement ===> ', plotAreaElement)
              
              // 检测图表类型
              if (plotAreaElement.getInnerElements('c:barChart') && plotAreaElement.getInnerElements('c:barChart').length > 0) {
                chartType = 'bar';
              } else if (plotAreaElement.getInnerElements('c:lineChart') && plotAreaElement.getInnerElements('c:lineChart').length > 0) {
                chartType = 'line';
              } else if (plotAreaElement.getInnerElements('c:pieChart') && plotAreaElement.getInnerElements('c:pieChart').length > 0) {
                chartType = 'pie';
              } else if (plotAreaElement.getInnerElements('c:areaChart') && plotAreaElement.getInnerElements('c:areaChart').length > 0) {
                chartType = 'area';
              } else if (plotAreaElement.getInnerElements('c:scatterChart') && plotAreaElement.getInnerElements('c:scatterChart').length > 0) {
                chartType = 'scatter';
              } else if (plotAreaElement.getInnerElements('c:radarChart') && plotAreaElement.getInnerElements('c:radarChart').length > 0) {
                chartType = 'radar';
              }
            }
            ...
            // 解析图表标题
            var chartTitle = null;
            var titleElements = this.readXml.getElementsByTagName('c:chartSpace/c:chart/c:title/c:tx/c:rich/a:p/a:r/a:t', src);
            if (titleElements == null || titleElements.length == 0) {
              titleElements = this.readXml.getElementsByTagName('c:chartSpace/c:chart/c:title/c:tx/c:strRef/c:strCache/c:pt/c:v', src);
            }
            if (titleElements != null && titleElements.length > 0) {
              chartTitle = titleElements[0].value;
            }
  ...
  LuckySheet.prototype.generateConfigRowLenAndHiddenAddCell = function () {
    var rows = this.readXml.getElementsByTagName(
      'sheetData/row',
      this.sheetFile
    );
    var cellOtherInfo = {};
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (this.isInitialCell) {
        ...
        for (var key in cells) {
          var cell = cells[key];
          var cellValue = new LuckySheetCelldata(
            cell,
            this.styles,
            this.sharedStrings,
            this.mergeCells,
      ...
      .then(
        function (zip) {
          ...
          zip.forEach(function (relativePath, zipEntry) {
            ...
            if (
              suffix in
              { png: 1, jpeg: 1, jpg: 1, gif: 1, bmp: 1, tif: 1, webp: 1 }
            ) {
              fileType = 'base64';
            } else if (suffix == 'emf') {
